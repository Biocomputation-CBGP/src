...
parser = argparse.ArgumentParser(description = description_message, epilog = epilog_message, formatter_class=argparse.RawTextHelpFormatter)
...
Files that will have the query strand(s) to search agains the genome sequence
                    """)
parser.add_argument("typeReads", choices = ["txt","seq"],
                    help = """
Extension that the reads will have, the vary for one sequence bussiness to others
For now only txt and seq extensions file are allowed
                    """)
...
type_files = args.typeReads #txt and seq for now
...
if os.path.isdir(final_directory):
    txt = input("------------------------------------------------------------------------------------------------------------------------------\n\n The directory '"+final_directory+"' is going to be overwrited, are you sure? [y/n]:  ")
    if txt == "y":
        os.system("rm -r "+final_directory)
        os.system("mkdir -p "+final_directory)
        # print("------------------------------------------------------------------------------------------------------------------------------")
    elif txt == "n":
        print("""\n------------------------------------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------------------------------------------------------""")
    ...
    os.system("mkdir "+final_directory) #Directory in which we are going to store the results
...
command_blastn_tabular_output = "blastn -query all_reads_merged.fna -db "+database+" -out "+final_directory+"/all_seq_aligned.tsv -outfmt '6 "+" ".join(columns_seq_alig)+"'"
if args.filesOut == "sam" or args.filesOut == "all":
    command_blastn_SAM_output = "blastn -query all_reads_merged.fna -db "+database+" -out "+final_directory+"/all_seq_aligned.sam -outfmt '17 "+" ".join(columns_seq_alig)+"'"
...
\n Adding Identity Columns to table with the information in {args.identity}""")
    ...
    if args.typeReads == "txt":
        regex_exp = r"_(\D+)(\d+)_"
    elif args.typeReads == "seq":
        regex_exp = r"\+(\D+)(\d+)_"
